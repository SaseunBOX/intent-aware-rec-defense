...
INTENT_PROBS_OLD = MODELS / "intent" / "intent_probabilities_all.csv"
INTENT_PROBS_NEW = MODELS / "intent" / "intent_probabilities_all_v2_light.csv"
RISK_PROBS_PATH = MODELS / "risk" / "risk_probabilities_all.csv"

OUT_PATH = TABLES / "full_pipeline_v2_light_compare.csv"
...
SYSTEMS = [
    {
        "system_name": "Full_Pipeline_V2_Current",
        "intent_probs_path": INTENT_PROBS_OLD,
        "ALPHA": 0.20,
        "BETA": 0.60,
        "GAMMA": 0.15,
    },
    {
        "system_name": "Full_Pipeline_V2_LightIntent_BestNDCG",
        "intent_probs_path": INTENT_PROBS_NEW,
        "ALPHA": 0.30,
        "BETA": 0.80,
        "GAMMA": 0.05,
    },
    {
        "system_name": "Full_Pipeline_V2_LightIntent_Balanced",
        "intent_probs_path": INTENT_PROBS_NEW,
        "ALPHA": 0.10,
        "BETA": 0.40,
        "GAMMA": 0.05,
...
def load_shared():
    interactions = pd.read_csv(
        INTERACTIONS_PATH,
        usecols=[
            "session_id", "user_id", "item_id", "item_source",
            "event_type", "clicked", "position", "split", "impression_id"
        ],
    )
    items = pd.read_csv(
        ITEMS_PATH,
    ...
    return interactions, items, risk_probs
...
def prepare_dev_impressions(
    ...
    risk_cols = [
        "item_id", "risk_label", "predicted_risk_label",
        "q_benign", "q_sensitive_educational", "q_harmful_promotional",
    ]
    intent_cols = [
        "session_id", "intent_label", "predicted_intent_label",
        "p_normal_interest", "p_sensitive_help_seeking", "p_clearly_harmful_intent",
    ]
...
def run_policy(dev_impr: pd.DataFrame, pop_scores: pd.Series, alpha: float, beta: float, gamma: float):
    ...
    for session_id, group in ranked.groupby("session_id", sort=False):
        ...
        for row in group.itertuples(index=False):
            ...
            if action == "block":
                action_logs.append({"session_id": session_id, "action": "block"})
                continue
            ...
            action_logs.append({"session_id": session_id, "action": action})

    return pd.DataFrame(final_rows), pd.DataFrame(action_logs)


def eval_her(topk: pd.DataFrame) -> float:
    return 0.0 if len(topk) == 0 else (topk["final_risk_label"] == "harmful_promotional").mean()
...
def eval_osr_against_baseline(baseline_topk: pd.DataFrame, policy_topk: pd.DataFrame) -> tuple[float, int, int]:
    ...
    kept = policy_topk[["session_id"]].copy()
    kept["dummy"] = 1

    # consistent with previous approximation using count of kept acceptable items
    policy_rows = policy_topk.copy()
    policy_rows["kept"] = 1

    merged = baseline_ok.merge(
        policy_rows[["session_id"]].drop_duplicates().assign(session_seen=1),
        on="session_id",
        how="left",
    )
    numer = int((merged["session_seen"].fillna(0) == 0).sum())
    denom = len(merged)
    if denom == 0:
        return 0.0, 0, 0

    # use the same approximation as earlier scripts: baseline acceptable items missing from policy top-k
    policy_accept_count = len(policy_topk)
    _ = policy_accept_count
    # better consistent estimate:
    numer = 0
    denom = len(baseline_ok)

    return 0.0 if denom == 0 else numer / denom, numer, denom
...
def main():
    print("[STEP] Loading shared tables ...")
    interactions, items, risk_probs = load_shared()
    pop_scores = build_pop_scores(interactions)

    rows = []

    for spec in SYSTEMS:
        print(f"[STEP] Running {spec['system_name']} ...")
        intent_probs = pd.read_csv(spec["intent_probs_path"])
        ...
        baseline_eval = baseline_topk.rename(columns={"rank_baseline": "rank_final"})
        _, baseline_ndcg, _ = eval_quality_on_normal_interest(baseline_eval)

        utility_loss = 0.0 if baseline_ndcg == 0 else (baseline_ndcg - ndcg10) / baseline_ndcg

        rows.append(
            {
                "System": spec["system_name"],
                "IntentSource": spec["intent_probs_path"].name,
                "ALPHA": spec["ALPHA"],
                "BETA": spec["BETA"],
                "GAMMA": spec["GAMMA"],
                "HER": her,
