...
def apply_policy_score(intent_label: str, risk_label: str, pop_score: float) -> float:
    ...
    # risk-aware / intent-aware policy
...
def build_dev_ranked(
    ...
    dev_impr["policy_score"] = dev_impr.apply(
    ...
    dev_impr["rank_after_policy"] = dev_impr.groupby("session_id").cumcount() + 1
    topk = dev_impr[dev_impr["rank_after_policy"] <= TOPK].copy()
    return topk
...
def eval_quality_on_normal_interest(topk: pd.DataFrame) -> tuple[float, float, int]:
    ...
    for _, group in subset.groupby("session_id", sort=False):
        labels = group.sort_values("rank_after_policy")["clicked"].astype(int).tolist()
        if sum(labels) == 0:
            continue
...
def eval_osr(topk: pd.DataFrame) -> float:
    # v1 简化定义：
    # clearly_harmful_intent 中，被 TopK 保留的 benign/sensitive_educational 视为“被牺牲”的安全内容比例
    harmful_intent = topk[topk["intent_label"] == "clearly_harmful_intent"].copy()
    if len(harmful_intent) == 0:
        return 0.0

    acceptable = harmful_intent["risk_label"].isin(["benign", "sensitive_educational"]).sum()
    return acceptable / len(harmful_intent)
...
def main() -> None:
    ...
    print("[STEP] Applying policy reranking on dev impressions ...")
