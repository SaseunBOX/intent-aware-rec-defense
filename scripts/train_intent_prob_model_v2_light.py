...
MODEL_PATH = MODELS / "intent_model_v2_light.joblib"
LABEL_MAP_PATH = MODELS / "intent_label_mapping_v2_light.json"
ALL_PROBS_PATH = MODELS / "intent_probabilities_all_v2_light.csv"
METRICS_PATH = TABLES / "intent_model_v2_light_metrics.csv"
TOP_FEATURES_PATH = TABLES / "intent_model_v2_light_top_features.csv"
REPORT_PATH = TABLES / "intent_model_v2_light_classification_report.txt"
...
HARMFUL_KEYWORDS = {
    ...
    "violent", "violence", "self harm", "extremist", "radical",
    "harass", "harassment",
}
...
def build_matrices(df_train: pd.DataFrame, df_dev: pd.DataFrame):
    ...
    X_train = hstack([X_word_train, X_num_train]).tocsr()
    X_dev = hstack([X_word_dev, X_num_dev]).tocsr()

    return word_vectorizer, scaler, num_cols, X_train, X_dev
...
def train_model(feature_df: pd.DataFrame):
    ...
    word_vectorizer, scaler, num_cols, X_train, X_dev = build_matrices(df_train, df_dev)
    ...
    print("[MODEL] training SGDClassifier(loss='log_loss') ...")
    ...
    X_full = hstack([X_word_full, X_num_full]).tocsr()
    ...
    # top features
    feature_names = list(word_vectorizer.get_feature_names_out()) + num_cols
    coef = clf.coef_
    top_rows = []
...
def main() -> None:
    ...
    print("[STEP] Building session-level features (v2 light) ...")
    ...
    print("[STEP] Training intent model v2 light ...")
    artifact, metrics_df, probs_df, top_features_df, report_txt = train_model(feature_df)
    ...
    with open(LABEL_MAP_PATH, "w", encoding="utf-8") as f:
        json.dump(
            {
                "label_order": LABEL_ORDER,
                "overall_accuracy": artifact["overall_accuracy"],
    ...
    print()
    print("[DONE] Intent model v2 light outputs saved:")
    print(f"  - {MODEL_PATH}")
